...
# ── Remote session flag ───────────────────────────────────────────────────────
# Read remote_sessions from config.json (opt-in, default false).
$remoteFlag = ""
...
if (Test-Path $configPath) {
    try {
        ...
        Log "[Dayarc] WARNING: Could not read config.json for remote_sessions flag: $_"
    }
}

Log "[Dayarc] Trigger: $trigger | Date: $($today.ToString('yyyy-MM-dd')) | Agent: $agentName | Remote: $($remoteFlag -ne '')"
