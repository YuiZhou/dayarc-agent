...
7. **Write all text fields (`themes[].label`, `themes[].progress`, `accomplishments[]`, `stuck[].description`, `focus_next[]`) in the language specified by `config.preferences.locale` (default: `en`). For `zh`: use Simplified Chinese professional tone.**
